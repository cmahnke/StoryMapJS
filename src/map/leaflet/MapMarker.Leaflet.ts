...
export default class LeafletMapMarker extends MapMarker {
    ...
    _createIcon() {
        return L.icon({ iconUrl: this._custom_icon.url, iconSize: this._custom_icon.size, iconAnchor: this._custom_icon.anchor });
    }

    _createImage(active?): any { // TODO: calculate shadow dimensions
        var className = active ? "vco-mapmarker-image-icon-active" : "vco-mapmarker-image-icon";
        return L.icon({ iconUrl: this._custom_image_icon, iconSize: [48, 48], iconAnchor: [24, 48], shadowSize: [68, 95], shadowAnchor: [22, 94], className: className });
    }

    _createDefaultIcon(active?): any {
