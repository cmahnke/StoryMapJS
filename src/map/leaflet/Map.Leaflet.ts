...
export default class Leaflet extends Map {
	...
	_createMap() {
		...
		this._map = L.map(this._el.map, {scrollWheelZoom:false, zoomControl:!this.options.map_mini});
	...
	_createBackgroundMap(tiles) {
		...
		if (!this._image_layer) {
			...
			this._image_layer = L.layerGroup();
		...
		if (tiles) {
			...
			for (let x in tiles) {
				...
				image = L.imageOverlay(tile.url, [tile.pos.start, tile.pos.end]);
	...
	_markerOverview() {
		...
		if (this.options.map_type == "zoomify" && this.options.map_as_image) {
			...
			if (this.options.map_center_offset && this.options.map_center_offset.left != 0 || this.options.map_center_offset.top != 0) {
				var the_bounds 	= L.latLngBounds(this.bounds_array);
