...
export default class OpenLayers extends Map {
	...
	_createMap() {
		...
		this._line.setStyle(new Style({ stroke: new Stroke({ color: this.options.line_color_inactive, width: this.options.line_weight }) }));
		this._map.addLayer(this._line);
		this._line.setOpacity(this.options.line_opacity);

		// Create Active Line
		this._line_active = this._createLine();
		this._line_active.setStyle(new Style({ stroke: new Stroke({ color: this.options.line_color, width: this.options.line_weight }) }));
		...
		if (attribution) {
			this._el.map.insertAdjacentHTML("beforeend", `<div class="vco-map-attribution">${attribution}</div>`);
		}
	}

	_getAttribution(map_type) {
		const parts = ["<a href='https://storymap.knightlab.com/' target='_blank' class='vco-knightlab-brand'><span>&#x25a0;</span> StoryMapJS</a>"];
		if (map_type === "osm" || map_type === "" || map_type.startsWith("osm")) {
			parts.push("© <a target='_blank' href='https://www.openstreetmap.org/copyright'>OpenStreetMap</a> contributors");
		}
		return parts.join(" | ");
	}
	...
	_createTileLayer(map_type) {
		...
		switch (_map_type_arr[0]) {
			...
			case 'stamen':
				...
				return new TileLayer({ source: new XYZ({ url: "https://tiles.stadiamaps.com/tiles/stamen_toner_lite/{z}/{x}/{y}.png", attributions: [] }) });
			...
			case 'iiif': {
				...
				fetch(this.options.iiif.url)
					...
					.catch((err) => console.error("IIIF info.json could not be loaded:", this.options.iiif.url, err?.stack || err));
				return iiif_layer;
			}

			case 'http':
			case 'https':
				return new TileLayer({ source: new XYZ({ url: map_type, attributions: [], crossOrigin: "anonymous" }) });

			case 'ch-watercolor':
				return new TileLayer({ source: new XYZ({ url: "https://watercolormaps.collection.cooperhewitt.org/tile/watercolor/{z}/{x}/{y}.jpg", attributions: [], maxZoom: 16 }) });
	...
	_getAllMarkersBounds(markers_array) {
		...
		for (let i = 0; i < markers_array.length; i++) {
			if (markers_array[i].data.real_marker) {
				coords.push([markers_array[i].data.location.lon, markers_array[i].data.location.lat]);
			}
		}
		return coords;
	}
	...
	_calculateMarkerZooms() {
		for (let i = 0; i < this._markers.length; i++) {
			...
			if (this._markers[i].data.location) {
				...
				const prev_marker = i > 0 ? this._markers[i-1].location() : this._getMapCenter(true);
				const next_marker = i < (this._markers.length - 1) ? this._markers[i+1].location() : this._getMapCenter(true);
	...
	_createLine(d?): any {
		return new VectorLayer({
			...
			style: new Style({ stroke: new Stroke({ color: this.options.line_color, width: this.options.line_weight }) }),
		});
	}
	...
	_replaceLines(line, array) {
		...
		const source = line.getSource();
		source.clear();
		source.addFeature(new Feature({ geometry: new LineString(this._markerCoordsToViewCoords(pts)) }));
	}
	...
	_getMapCenter(offset) {
		const center = toLonLat(this._map.getView().getCenter(), this._map.getView().getProjection());
	...
	_getBoundsZoom(origin, destination, correct_for_center) {
		const coords = [[origin.lon !== undefined ? origin.lon : origin.lng, origin.lat], [destination.lon, destination.lat]];
		const view_coords = this._markerCoordsToViewCoords(coords);
		const extent = boundingExtent(view_coords);
		...
		if (this.options.map_type == "iiif" && this.options.map_as_image) {
			...
			if (this.options.map_center_offset && this.options.map_center_offset.left != 0 || this.options.map_center_offset.top != 0) {
				if (this.bounds_array && this.bounds_array.length) {
					...
					const zoom = Math.max(0, Math.round(this._map.getView().getZoomForResolution(resolution)) - 1);
