...
const storymapAnimate = (function () {
    ...
    function toHex(c) {
        ...
        return (m ? rgb(m[1], m[2], m[3]) : c).replace(/#(\w)(\w)(\w)$/, "#$1$1$2$2$3$3"); // short skirt to long jacket
    }
    ...
    */
        ...
        if (k === "transform") {
            ...
            for (const t in begin[i][k]) {
                v[t] =
                    t in end[i][k]
                        ? Math.round(
                              ((end[i][k][t] - begin[i][k][t]) * pos + begin[i][k][t]) * thousand,
                          ) / thousand
                        : begin[i][k][t];
            }
            return v;
    ...
    function morpheus(elements, options) {
        ...
        return tween.apply(els, [
            duration,
            function (this: any, pos: any, v: any, xy: any) {
