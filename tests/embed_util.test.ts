...
test("sanitizeBlockquote keeps quote structure and text", () => {
	const html = renderedHTML(
		sanitizeBlockquote("<blockquote><p>Truth is <em>rarely</em> pure.</p><cite>Oscar Wilde</cite></blockquote>") as Node
	);
	expect(html).toBe("<blockquote><p>Truth is <em>rarely</em> pure.</p><cite>Oscar Wilde</cite></blockquote>");
});
...
test("sanitizeBlockquote strips event handler and style attributes", () => {
	const html = renderedHTML(
		sanitizeBlockquote('<blockquote onclick="alert(1)" style="color:red">quote</blockquote>') as Node
	);
	expect(html).toBe("<blockquote>quote</blockquote>");
});
...
test("sanitizeBlockquote keeps links with valid href, drops javascript: href", () => {
	const good = renderedHTML(
		sanitizeBlockquote('<blockquote><a href="https://example.com/">link</a></blockquote>') as Node
	);
	expect(good).toBe('<blockquote><a href="https://example.com/" target="_blank">link</a></blockquote>');

	const bad = renderedHTML(
		sanitizeBlockquote('<blockquote><a href="javascript:alert(1)">link</a></blockquote>') as Node
	);
